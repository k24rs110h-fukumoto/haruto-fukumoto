...
public class CharacterMakeManager : MonoBehaviour
{
    ...
    [SerializeField] private Image[] standingSelectedImage;
    ...
    private void UpdateSelectedFrame()
    {
        ...
        for (int i = 0; i < standingSelectedImage.Length; i++)
        {
            if (standingSelectedImage[i] != null)
            {
                standingSelectedImage[i].gameObject.SetActive(i == selectedIconNumber);
            }
        }
    }
